...
// particularly with sockets bound to ports. It also helps memory analysis with tools like valgrind,
// but valgrind really does not like programs that use OpenSSL. A version of this software that does
// not use OpenSSL may be worth creating just so valgrind gives reliable and clear data.
void free_all(Resources *resources) {
  if (resources->epoll_fd > 0) {
    close(resources->epoll_fd);
  }
  if (resources->socket_fd > 0) {
    close(resources->socket_fd);
  }
...
// This function does not take ownership of resources. That is the entire point, since it removes
// the possibility of accidentally not freeing a resource when this function exits.
//
// This function uses many calls that are extremely specific to linux, so it not very portable.
// Portable networking is possible via a networking layer, but that is very complex to do.
int main_protected(Resources *resources, int argc, char *argv[]) {
  ...
  { // Initialize the socket first, since if the socket is unavailable the server cannot start.
    resources->socket_fd = socket(AF_INET, SOCK_STREAM, 0);
    if (resources->socket_fd < 0) {
    ...
    if (bind(resources->socket_fd, cast(struct sockaddr *, &server_addr), sizeof(server_addr)) <
        0) {
      perror("Failed to bind a TCP socket\n");
      return -1;
    }
    if (listen(resources->socket_fd, TCP_BACKLOG) < 0) {
      perror("Failed to bind a TCP socket\n");
      return -1;
    }
    ...
    if (epoll_ctl(resources->epoll_fd, EPOLL_CTL_ADD, resources->socket_fd, &add_sock) < 0) {
      perror("Call to epoll_ctl failed");
      return -1;
    }
  }

  { // Get back to opening and reading the file or dir at path
    ...
    if (S_ISDIR(path_stat.st_mode)) {
      // The given path is a directory, we will attempt to trust every valid certificate found
      // within it.
  ...
  // The server is fully initialized so we can now start waiting for connections.
  ...
  while (true) {
    ...
    for_each_in(struct epoll_event, event, events, res) {
      ...
      if (idx == MAX_INT32) {
        ...
        int conn_fd =
            accept(resources->socket_fd, cast(struct sockaddr *, &clien_addr), &clien_addr_size);
        if (conn_fd < 0) {
